...
def parse_pubs(generator, refs_file, style=None):
    if style is None:
...
def add_publications(generator):
    if "PUBLICATIONS_SRC" in generator.settings:
        ...
        try:
            publications, publications_lists = parse_pubs(generator, refs_file)
...
def tutorial_publications(generator, content):
    ...
    if "bibtex" in content.metadata:
        ...
        try:
            publications, publications_lists = parse_pubs(generator, refs_file, unsrt.Style())
        except ImportError as e:
            logger.warn(f"`pelican_bib` failed to load {str(e)}")
            return
        except PybtexError as e:
